    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
            <div class="flex items-center gap-4">
                <div class="bg-linear-to-br from-gray-600 to-gray-800 w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg">
        ...
        {{-- Stats Cards --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8" wire:loading.class="opacity-50 transition-opacity">
            {{-- Consumo actual --}}
            <div class="bg-linear-to-br from-gray-700 to-gray-900 rounded-2xl p-6 text-white shadow-lg">
                ...
                @foreach($equipmentList->groupBy(fn($eq) => $eq->category->name ?? 'Sin categoría') as $categoryName => $items)
                    ...
                    @foreach($items as $eq)
                        @php
                            ...
                            $monthlyCost    = $monthlyKwh * $averageTariff;
                        @endphp
                        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors {{ $eq->is_standby ? '' : 'opacity-60 grayscale-[0.5]' }}">
                            {{-- Equipo info --}}
                            <div class="flex items-center gap-3 flex-1">
                                <div class="w-10 h-10 {{ $eq->is_standby ? 'bg-amber-100' : 'bg-emerald-100' }} rounded-lg flex items-center justify-center shrink-0">
                                    <i class="bi bi-plug {{ $eq->is_standby ? 'text-amber-600' : 'text-emerald-600' }}"></i>
                                </div>
                                <div class="min-w-0">
                                    <p class="font-medium text-gray-900 text-sm truncate">{{ $eq->name }}</p>
                                    <p class="text-xs text-gray-400">{{ $eq->room->name ?? '-' }} · {{ $standbyPowerW }}W · {{ number_format($standbyHours, 0) }}h en espera</p>
                                </div>
                            </div>
                            ...
                            <div class="text-right mr-8 hidden sm:block">
                                @if($eq->is_standby)
                                    ...
                                    <p class="text-xs text-red-500 font-medium">${{ number_format($monthlyCost, 0, ',', '.') }}/mes</p>
                                @else
                                    <p class="text-sm text-emerald-600 font-bold">Ahorrando</p>
                            ...
                            <div class="flex items-center">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer"
                                        ...
                                        wire:click="toggleStandby({{ $eq->id }})">
        ...
        <x-alert type="info" class="mt-6">
            <div class="flex items-start gap-3">
                ...
                <div class="text-sm">
                    <strong>¿Cómo funciona?</strong> Marcá los equipos que dejás enchufados. El sistema calcula el consumo fantasma y el ahorro potencial al desenchufarlos cuando no los usás.
                </div>
            </div>
        </x-alert>
    </div>
