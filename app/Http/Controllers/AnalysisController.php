...
{
    use GroupsInvoices, HasActiveEntity;
    ...
    private function getPeriodAnalytics(Entity $entity, array $period)
    {
        ...
        $climateData = app(ClimateService::class)->loadDataForDateRange($entity, $period['start_date'], $period['end_date']);
     ...
     */
    private function getTimeEvolutionData(Entity $entity, Collection $periods)
    {
        if ($periods->isEmpty()) {
            return [];
        }
    ...
    public function equipmentCost(Request $request)
    {
        ...
        foreach ($periods as $p) {
            ...
            $pLabel = Carbon::parse($p['end_date'])->locale('es')->translatedFormat('M y');
            $pDays = Carbon::parse($p['start_date'])->diffInDays(Carbon::parse($p['end_date'])) + 1;
        ...
        foreach ($rooms as $room) {
            foreach ($room->equipment as $equipment) {
                ...
                $item = [
                    ...
                    'cycles_per_period' => ($usage instanceof EquipmentUsage) ? $usage->cycles_per_period : ($usage['cycles_per_period'] ?? null),
    ...
    public function saveContextOnly(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $invoice) {
            foreach ($request->usages as $eqId => $data) {
                ...
                $equipment = Equipment::query()->find($eqId);
                if ($equipment) {
    ...
    public function showEngineResults(Request $request, Invoice $invoice)
    {
        ...
        return Inertia::render('Analisis/EngineResults', [
            ...
            'period' => [
                'start_date' => $invoice->start_date,
                'end_date' => $invoice->end_date,
                'days' => Carbon::parse($invoice->start_date)->diffInDays(Carbon::parse($invoice->end_date)),
    ...
    private function isCritical(Equipment $equipment): bool
    {
