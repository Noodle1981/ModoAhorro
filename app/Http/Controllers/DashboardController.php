...
class DashboardController extends Controller
{
    ...
    public function home(Request $request)
    {
        ...
        if ($currentEntity) {
            $invoices = Invoice::whereHas('contract', fn ($q) => $q->where('entity_id', $currentEntity->id))->get();
            if ($invoices->isNotEmpty()) {
                $stats['has_data'] = true;

                $latestInvoice = $invoices->sortByDesc('end_date')->first();
                $totalDays = Carbon::parse($latestInvoice->start_date)->diffInDays(Carbon::parse($latestInvoice->end_date)) ?: 30;
